...
- Chaque projet doit être analysé selon SA propre réalité factuelle. INTERDICTION d'inventer des rebrandings, des pivots stratégiques ou des labels (ex: UEX, Layer 2, DEX) s'ils ne s'appliquent pas REELLEMENT au projet étudié.
- Pour les tokens d'exchanges (ex: CET, BGB, BNB, KCS) : Analyse le modèle exact de la plateforme hôte (CEX classique, DEX, UEX, fonctions de burn, réduction de frais, staking, launchpad). Ne colle PAS d'étiquette générique si l'entreprise n'a pas officiellement opéré ce changement.
